# 프로젝트 제목: 같이 달램

![image](https://github.com/user-attachments/assets/421ff487-2472-4b00-bd7c-cc37202a982a)

[배포 사이트 ](https://next-media-mate.vercel.app/)

## 프로젝트 소개

- 유저가 바쁜 일상 속 휴식을 위한 다양한 모임을 탐색하고 참여하며, 직접 모임을 개설하고 리뷰를 생성할 수 있는 서비스입니다.

## 개발 인원 및 개발 기간

- 개발 인원 : 4명

- 개발 기간 : 2024.09.04 ~ 2024.10.20

## 기술 스택

<!--  -->

### ✔️ Environment

<img src="https://img.shields.io/badge/visualstudio-297ACC?style=for-the-badge&logo=visualstudio&logoColor=white"> <img src="https://img.shields.io/badge/git-F05032?style=for-the-badge&logo=git&logoColor=white"> <img src="https://img.shields.io/badge/github-black?style=for-the-badge&logo=github&logoColor=white">

### ✔️ Develpoment

<img src="https://img.shields.io/badge/reactquery-FF4154?style=for-the-badge&logo=reactquery&logoColor=black"> <img src="https://img.shields.io/badge/typescript-2F74C0?style=for-the-badge&logo=typescript&logoColor=white"> <img src="https://img.shields.io/badge/next.js-black?style=for-the-badge&logo=nextdotjs&logoColor=white"> <img src="https://img.shields.io/badge/Tailwind css-white?style=for-the-badge&logo=tailwindcss&logoColor=#06B6D4">
<img src="https://img.shields.io/badge/zustand-2D3748?style=for-the-badge&logo=zustand&logoColor=black">
<img src="https://img.shields.io/badge/React hook form-EC5990?style=for-the-badge&logo=React hook form&logoColor=black">

### ✔️ Deploy

<img src="https://img.shields.io/badge/vercel-2D3748?style=for-the-badge&logo=vercel&logoColor=black">

## 주요 기능

- 회원가입 및 로그인

- 모임 생성

- 모임 찾기 (필터링 기능)

- 모임 목록 찜 기능

- 모임 상세페이지 및 리뷰 확인

- 모든 리뷰 (필터링 기능)

- 마이 페이지

- 반응형 웹 디자인

## 화면 구성

### 회원가입 및 로그인 페이지

![회원가입2](https://github.com/user-attachments/assets/231484b9-6d3f-46e4-87f8-1be14e1cd50b)

- 회원 가입

- 로그인

### 홈 페이지

![홈페이지1](https://github.com/user-attachments/assets/147751b7-f720-4f2d-87c3-76270586eb0e)
![모임만들기](https://github.com/user-attachments/assets/69d9117d-3dce-4dd1-8d79-27848d4ae0f7)
![무한스크롤](https://github.com/user-attachments/assets/319baca3-a35c-4488-ac61-3796d26f8637)

- 모임 만들기

- 모임 찾기(위치, 날짜, 마감임박, 참여 인원 필터링)

### 상세 페이지

![상세페이지1](https://github.com/user-attachments/assets/387e1b55-ed9c-4167-981d-6f4687c8ede5)
![상세페이지2](https://github.com/user-attachments/assets/2038fc7f-e02f-4d11-8f92-b3b44e4fc849)

- 특정 모임 리뷰 확인

- 모임 참여 하기 및 취소

- 모임 취소 하기 및 공유(개최자)

### 찜한 모임 페이지

![찜한모임페이지](https://github.com/user-attachments/assets/70dbad56-7112-4c13-96c1-1657b7542774)

- 모임 찜 하기

- 찜 모임 목록 찾기(위치, 날짜, 마감임박, 참여 인원 필터링)

### 모든 리뷰 페이지

![리뷰페이지](https://github.com/user-attachments/assets/7b8abf93-e508-4978-981a-5891f830dc3d)

- 전체 모임 리뷰 확인

- 리뷰 필터링(위치, 날짜, 최신, 참여 인원, 리뷰 높은 순)

### 마이 페이지

![마이페이지수정](https://github.com/user-attachments/assets/ad3963b3-d781-40e1-ae1c-982825733cc8)
![마이페이지리뷰작성](https://github.com/user-attachments/assets/855df6ad-5566-4af7-953b-293a1a6cd826)

- 프로필 수정 하기

- 리뷰 작성 하기

- 모임 참여 취소 하기

## 아키텍처

![image](https://github.com/user-attachments/assets/5acb2d58-5468-4650-bafc-b9d2be8fd4e2)

### 역할분담

#### 조성훈

- 프로젝트 초기 세팅
- zustand-persist를 활용하여 localStorage 통해 찜 목록 관리
- 프로필 수정, 나의 모임, 나의 리뷰, 내가 만든 모임 구현
- react-query의 useInfiniteQuery를 사용하여 무한 스크롤 구현
- react-query의 setQueryData를 사용하여 데이터 관리
- jest, cypress로 마이페이지, 찜 기능 테스트 코드 작성

#### 김시몬

- 홈 페이지 , 상세 페이지 초기 10개 데이터 SSR 처리 이후 react-query의 useInfiniteQuery를 사용하여 데이터 무한 스크롤 기능(CSR) 구현
- 홈 페이지 , 상세 페이지, 찜 목록 페이지 필터링 기능 구현 (지역, 날짜, 마감 임박, 참여 인원, 4가지 타입(달램핏, 워케이션, 마인드풀니스, 오피스 스트레칭))
- 모임 생성, 모임 취소, 링크 공유 기능 구현(주최자) , 모임 참여 및 취소 기능 구현(참여자)
- 상세 페이지 리뷰 목록 페이지 네이션 기능 구현
- 로컬 Storage에 저장된 찜 목록 데이터를 이용하여 찜 목록 페이지 구현
- 홈페이지, 상세 페이지, 찜 목록 페이지 Cypress 테스트 코드 작성
- framer motion을 이용하여 모임 목록 스크롤 애니메이션, 필터링 탭 애니메이션 구현

#### 오동혁

- Zod와 React-Hook-Form을 이용하여 로그인 및 회원가입 기능 구현
- Axios interceptor를 이용하여 전체적인 에러 처리 구현
- 알림 메시지를 보여주는 Toast 컴포넌트 구현
- 서버와 클라이언트를 구분하여 Axios 인스턴스 구현
- 로그인 및 회원 가입 페이지 Cypress 테스트 코드 작성

#### 남윤하

- 모든 리뷰 페이지 필터링 기능 구현 (지역, 날짜, 최신순, 리뷰 높은 순, 참여 인원, 4가지 타입(달램핏, 워케이션, 마인드풀니스, 오피스 스트레칭))
- Zod와 React-Hook-Form을 이용하여 모임 만들기 모달 구현
- prefetchInfiniteQuery를 이용하여 모든 리뷰 페이지 구현
- 사용자 UX 향상을 위해 스켈레톤 구현
- Jest 유닛 테스트 코드 작성
- framer motion을 이용하여 필터링 탭 애니메이션 구현
